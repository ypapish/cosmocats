...
public class OrderService {
  ...
  public OrderDto createOrder(OrderDto orderDto) {
    ...
    OrderEntity order =
        OrderEntity.builder().orderUuid(UUID.randomUUID()).totalPrice(BigDecimal.ZERO).build();

    if (orderDto.getEntries() != null) {
      for (OrderEntryDto entryDto : orderDto.getEntries()) {
        ProductEntity product =
            productRepository
                .findByNaturalId(entryDto.getProduct().getProductId())
                .orElseThrow(
                    () ->
                        new RuntimeException(
                            "Product not found: " + entryDto.getProduct().getProductId()));

        OrderEntryEntity orderEntry =
