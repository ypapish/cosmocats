...
public class SecurityConfig {
  ...
  public SecurityFilterChain adminApiKeyFilterChain(HttpSecurity http) throws Exception {
    http.securityMatcher("/api/v1/admin/**", "/api/v1/admin/products/**")
        .csrf(csrf -> csrf.disable())
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
        .addFilterBefore(new CosmicKeyFilter(), UsernamePasswordAuthenticationFilter.class)
  ...
  public SecurityFilterChain docsFilterChain(HttpSecurity http) throws Exception {
    http.securityMatcher("/v3/api-docs/**", "/swagger-ui/**", "/swagger-ui.html", "/actuator/**")
        .authorizeHttpRequests(auth -> auth.anyRequest().permitAll());
