...
class CategoryServiceTest {
    ...
    void createCategory_WithValidData_ShouldCreateAndReturnCategory() {
        ...
        when(categoryRepository.findByName("Galactic Supplies"))
                .thenReturn(Optional.empty());
        when(categoryRepository.save(any(CategoryEntity.class)))
                .thenReturn(newCategoryEntity);
        when(categoryMapper.toCategoryDto(newCategoryEntity))
    ...
    void getCategoryById_WithExistingId_ShouldReturnCategory() {
        when(categoryRepository.findByNaturalId(categoryId))
                .thenReturn(Optional.of(testCategoryEntity));
        when(categoryMapper.toCategoryDto(testCategoryEntity))
    ...
    void getCategoryById_WithNonExistingId_ShouldThrowCategoryNotFoundException() {
        ...
        when(categoryRepository.findByNaturalId(nonExistingId))
                .thenReturn(Optional.empty());
    ...
    void getCategoryByName_WithNonExistingName_ShouldThrowCategoryNotFoundException() {
        when(categoryRepository.findByName("NonExistingCategory"))
                .thenReturn(Optional.empty());
    ...
    void getAllCategories_WithCategories_ShouldReturnCategoryList() {
        CategoryEntity secondCategory = CategoryEntity.builder()
                .categoryUuid(UUID.randomUUID())
                .name("Books")
                .build();

        CategoryDto secondCategoryDto = CategoryDto.builder()
                .categoryId(secondCategory.getCategoryUuid())
                .name("Books")
                .build();
        ...
        when(categoryMapper.toCategoryDtoList(categoryEntities))
    ...
    void getAllCategories_WhenNoCategories_ShouldReturnEmptyList() {
        ...
        when(categoryMapper.toCategoryDtoList(emptyList))
    ...
    void updateCategory_WithValidData_ShouldUpdateAndReturnCategory() {
        ...
        when(categoryRepository.findByNaturalId(categoryId))
                .thenReturn(Optional.of(testCategoryEntity));
        when(categoryRepository.findByName("Updated Electronics"))
                .thenReturn(Optional.empty());
        when(categoryRepository.save(any(CategoryEntity.class)))
                .thenReturn(updatedCategoryEntity);
        when(categoryMapper.toCategoryDto(updatedCategoryEntity))
    ...
    void updateCategory_WithDuplicateName_ShouldThrowCategoryAlreadyExistsException() {
        CategoryEntity existingCategoryWithSameName = CategoryEntity.builder()
                .categoryUuid(UUID.randomUUID())
    ...
    void searchCategoriesByName_WithMatchingName_ShouldReturnCategories() {
        CategoryEntity galaxyCategory = CategoryEntity.builder()
                .categoryUuid(UUID.randomUUID())
                .name("Galaxy Equipment")
                .build();

        CategoryEntity spaceCategory = CategoryEntity.builder()
                .categoryUuid(UUID.randomUUID())
                .name("Space Tools")
                .build();

        CategoryDto galaxyCategoryDto = CategoryDto.builder()
