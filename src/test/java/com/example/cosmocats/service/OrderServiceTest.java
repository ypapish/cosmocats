...
class OrderServiceTest {
    ...
    void setUp() {
        ...
        OrderEntryEntity orderEntry1 = OrderEntryEntity.builder()
                .product(testProduct1)
                .amount(2)
                .build();

        OrderEntryEntity orderEntry2 = OrderEntryEntity.builder()
        ...
        OrderEntryDto orderEntryDto1 = OrderEntryDto.builder()
                .product(productDto1)
                .amount(2)
                .build();

        OrderEntryDto orderEntryDto2 = OrderEntryDto.builder()
                .product(productDto2)
                .amount(1)
                .build();
    ...
    void createOrder_WithValidData_ShouldCreateAndReturnOrder() {
        ...
        when(productRepository.findByNaturalId(productId1))
                .thenReturn(Optional.of(testProduct1));
        when(productRepository.findByNaturalId(productId2))
                .thenReturn(Optional.of(testProduct2));
        when(orderRepository.save(any(OrderEntity.class)))
                .thenReturn(testOrderEntity);
        when(orderMapper.toOrderDto(testOrderEntity))
    ...
    void createOrder_WithNonExistingProduct_ShouldThrowException() {
        OrderEntryDto invalidEntry = OrderEntryDto.builder()
                .product(ProductDto.builder().productId(UUID.randomUUID()).build())
                .amount(1)
                .build();

        OrderDto invalidOrderDto = OrderDto.builder()
                .entries(List.of(invalidEntry))
                .totalPrice(100.0)
                .build();

        when(productRepository.findByNaturalId(any(UUID.class)))
    ...
    void createOrder_WithEmptyEntries_ShouldCreateOrderWithZeroTotal() {
        ...
        OrderEntity emptyOrderEntity = OrderEntity.builder()
                .id(1L)
                .orderUuid(orderId)
        ...
        OrderDto emptyOrderResultDto = OrderDto.builder()
                .id(orderId)
                .entries(List.of())
                .totalPrice(0.0)
                .build();
        ...
        when(orderRepository.save(any(OrderEntity.class)))
                .thenReturn(emptyOrderEntity);
        when(orderMapper.toOrderDto(emptyOrderEntity))
                .thenReturn(emptyOrderResultDto);
    ...
    void getOrderById_WithExistingId_ShouldReturnOrder() {
        when(orderRepository.findByNaturalId(orderId))
                .thenReturn(Optional.of(testOrderEntity));
        when(orderMapper.toOrderDto(testOrderEntity))
                .thenReturn(testOrderDto);
    ...
    void getOrderById_WithNonExistingId_ShouldThrowOrderNotFoundException() {
        ...
        when(orderRepository.findByNaturalId(nonExistingId))
                .thenReturn(Optional.empty());
    ...
    void getAllOrders_WithOrders_ShouldReturnOrderList() {
        ...
        when(orderMapper.toOrderDtoList(orderEntities))
    ...
    void getAllOrders_WhenNoOrders_ShouldReturnEmptyList() {
        ...
        when(orderMapper.toOrderDtoList(emptyList))
    ...
    void updateOrder_WithValidData_ShouldUpdateAndReturnOrder() {
        ...
        OrderEntity updatedOrderEntity = OrderEntity.builder()
                ...
                .entries(List.of(OrderEntryEntity.builder()
                        .product(testProduct1)
                        .amount(3)
                        .build()))
                .build();
        ...
        when(productRepository.findByNaturalId(productId1))
                .thenReturn(Optional.of(testProduct1));
        when(orderRepository.save(any(OrderEntity.class)))
                .thenReturn(updatedOrderEntity);
        when(orderMapper.toOrderDto(updatedOrderEntity))
    ...
    void updateOrder_WithNonExistingId_ShouldThrowOrderNotFoundException() {
        ...
        assertThrows(OrderNotFoundException.class,
            () -> orderService.updateOrder(nonExistingId, testOrderDto));
    ...
    void updateOrder_WithNonExistingProduct_ShouldThrowException() {
        ...
        when(orderRepository.findByNaturalId(orderId))
                .thenReturn(Optional.of(testOrderEntity));
        when(productRepository.findByNaturalId(nonExistingProductId))
                .thenReturn(Optional.empty());
    ...
    void calculateOrderTotal_WithNonExistingOrder_ShouldThrowOrderNotFoundException() {
        ...
        assertThrows(OrderNotFoundException.class,
            () -> orderService.calculateOrderTotal(nonExistingId));
    ...
    void getOrdersWithTotalGreaterThan_ShouldReturnFilteredOrders() {
        OrderEntity smallOrder = OrderEntity.builder()
                .orderUuid(UUID.randomUUID())
                .totalPrice(BigDecimal.valueOf(500.0))
                .entries(List.of())
                .build();

        OrderEntity largeOrder = OrderEntity.builder()
                .orderUuid(UUID.randomUUID())
    ...
    void calculateTotalPrice_WithMultipleEntries_ShouldReturnCorrectTotal() {
        OrderEntryEntity entry1 = OrderEntryEntity.builder()
                .product(ProductEntity.builder().price(BigDecimal.valueOf(100.0)).build())
                .amount(2)
                .build();

        OrderEntryEntity entry2 = OrderEntryEntity.builder()
