...
class ProductControllerIntegrationTest extends AbstractIt {
  ...
  void setUp() {
    ...
    booksCategory = CategoryEntity.builder().categoryUuid(UUID.randomUUID()).name("Books").build();
  ...
  void getProductById_WithExistingId_ShouldReturnProduct() {
    ...
    mockMvc
        .perform(
            get("/api/v1/products/{id}", savedProduct.getProductUuid())
  ...
  void getProductsByCategory_WithExistingCategory_ShouldReturnProducts() {
    ...
    mockMvc
        .perform(get("/api/v1/products/category/Electronics").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.products.length()").value(2))
  ...
  void getProductsByCategory_WithNonExistingCategory_ShouldReturnEmptyList() {
    mockMvc
        .perform(
            get("/api/v1/products/category/NonExistingCategory").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.products.length()").value(0));
  }
  ...
  void getProductsByCategory_WithEmptyCategory_ShouldReturnEmptyList() {
    mockMvc
        .perform(get("/api/v1/products/category/EmptyCategory").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.products.length()").value(0));
  }
}
