...
class CategoryControllerIntegrationTest extends AbstractIt {
    ...
    void getCategoryById_WithExistingId_ShouldReturnCategory() {
        ...
        mockMvc.perform(get("/api/v1/categories/{id}", savedCategory.getCategoryUuid())
    ...
    void getCategoryByName_WithExistingName_ShouldReturnCategory() {
        ...
        mockMvc.perform(get("/api/v1/categories/search/{name}", "Galaxy Equipment")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void getCategoryByName_WithNonExistingName_ShouldReturnNotFound() {
        mockMvc.perform(get("/api/v1/categories/search/{name}", "NonExistingCategory")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound());
    }
    ...
    void updateCategory_WithValidData_ShouldUpdateCategory() {
        ...
        CategoryDto updateDto = CategoryDto.builder()
                .name("Updated Galactic Name")
                .build();

        mockMvc.perform(put("/api/v1/categories/{id}", savedCategory.getCategoryUuid())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateDto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Updated Galactic Name"));

        CategoryEntity updatedCategory = categoryRepository.findByNaturalId(savedCategory.getCategoryUuid()).get();
    ...
    void updateCategory_WithDuplicateName_ShouldReturnConflict() {
        ...
        mockMvc.perform(put("/api/v1/categories/{id}", savedCategory2.getCategoryUuid())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateDto)))
                .andExpect(status().isConflict());
    }
    ...
    void deleteCategory_WithExistingId_ShouldDeleteCategory() {
        ...
        mockMvc.perform(delete("/api/v1/categories/{id}", savedCategory.getCategoryUuid()))
